...
public class ElementDAOImpl implements ElementDAO {
    ...
    public List<Element> getElementModulesByModule(String code_module) {
        List<Element> elementModules = new ArrayList<>();

        try (PreparedStatement statement = Connexion.getInstance().getConnection().prepareStatement("SELECT * FROM ElementModule WHERE code_module=?")) {
            statement.setString(1, code_module);
            try (ResultSet resultSet = statement.executeQuery()) {
                while (resultSet.next()) {
                    Element elementModule = extractElementModuleFromResultSet(resultSet);
